...
namespace VideoSource
{
	...
	public class VideoStream : IVideoSource
	{
			...
			while ((!failed) && (!stopEvent.WaitOne(0, true)))
			{
				try
				{
					...
					if (sg.GetConnectedMediaType(mt) == 0)
					{
						...
						grabber.Width = vih.BmiHeader.Width/3;
						grabber.Height = vih.BmiHeader.Height/3;
						mt.Dispose();
					}
		...
		private class Grabber : ISampleGrabberCB
		{
			...
			{
				// create new image
				System.Drawing.Bitmap img = new Bitmap(width, height, PixelFormat.Format1bppIndexed);
